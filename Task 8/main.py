...
from src.model import VisionTransformer, create_vit
...
# Default configuration (matches baseline for fair comparison)
DEFAULT_CONFIG = {
    'img_size': 28,
    'patch_size': 7,
    'in_channels': 1,
...
def main(args):
    ...
    model = create_vit(DEFAULT_CONFIG)
    n_params = count_parameters(model)
    print(f"Model parameters: {n_params:,}")
    
    # Print model architecture summary
    print(f"\nModel Architecture:")
    print(f"  - Patch size: {DEFAULT_CONFIG['patch_size']}")
    print(f"  - Number of patches: {(28//DEFAULT_CONFIG['patch_size'])**2}")
    print(f"  - Embedding dim: {DEFAULT_CONFIG['embed_dim']}")
    print(f"  - Transformer depth: {DEFAULT_CONFIG['depth']}")
    ...
    # Save model
    if args.save_model:
        trainer.save_model(os.path.join(results_dir, 'vit_mnist.pt'))
    ...
    print(f"Baseline: 97.44%")
    print(f"Difference: {(test_acc - 0.9744)*100:+.2f}%")
    print("=" * 60)
